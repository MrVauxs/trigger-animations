...
if (!version || !versionPrefix || !changelogPath) {
  console.error("Usage: extract-changelog.mjs <version> <version_prefix> <changelog_path>");
  process.exit(1);
}
...
for (let i = start; i < lines.length; i++) {
  if (lines[i].startsWith(versionPrefix) && lines[i] !== lines[start - 1]) break;
  sectionLines.push(lines[i]);
}

// Trim leading/trailing blank lines while preserving internal whitespace
let trimStart = 0;
while (trimStart < sectionLines.length && sectionLines[trimStart].trim() === "") trimStart++;
let trimEnd = sectionLines.length;
while (trimEnd > trimStart && sectionLines[trimEnd - 1].trim() === "") trimEnd--;
...
if (outputFile) {
  ...
  appendFileSync(outputFile, `changelog<<${delimiter}\n${changelog}\n${delimiter}\n`);
} else {
